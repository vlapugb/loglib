...
namespace logger {
    ...
    std::string iso8601_utc(const std::uint64_t epoch_ms) noexcept {
        ...
        std::tm tm{};
        if (gmtime_r(&tt, &tm) == nullptr) return "1970-01-01T00:00:00Z";
        char buf[21];
        const int n = std::snprintf(buf, sizeof(buf), "%04d-%02d-%02dT%02d:%02d:%02dZ",
                              tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                              tm.tm_hour, tm.tm_min, tm.tm_sec);
        if (n != 20) return "1970-01-01T00:00:00Z";
    ...
    bool split_host_port(std::string_view in, std::string &host, std::uint16_t &port) noexcept {
        ...
        if (in.find(':', p + 1) != std::string_view::npos) return false;
        const std::string_view h = in.substr(0, p);
        const std::string_view ps = in.substr(p + 1);
        if (h.empty() || ps.empty()) return false;
        unsigned long v = 0;
        for (const char c: ps) {
